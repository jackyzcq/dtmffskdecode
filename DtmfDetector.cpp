...
static void goertzel_filter(INT16 Koeff0, INT16 Koeff1, const INT16 arraySamples[], INT32 *Magnitude0, INT32 *Magnitude1, UINT32 COUNT)
{
    ...
    // Vk2_1    prev_prev (second frequency)
...
const INT16 DtmfDetector::CONSTANTS[COEFF_NUMBER] = {
    27860,  // 0: 697Hz, harmonics include: 78Hz, 235Hz, 3592Hz 
    26745,  // 1: 770Hz, apparently a high G, harmonics: 78Hz
    25529,  // 2: 852Hz, harmonics: 78Hz 
    24216,  // 3: 941Hz, harmonics: 78Hz, 235Hz, 314Hz
    19747,  // 4: 1209Hz, harmonics: 78Hz, 235Hz, 392Hz, 3529Hz
    16384,  // 5: 1366Hz, harmonics: 78Hz 
    12773,  // 6: 1477Hz, harmonics: 78Hz, 2980Hz
    8967,   // 7: 1633Hz, harmonics: 314Hz, 392Hz
...
INT32 DtmfDetector::dialTonesToOhersDialTones = 1;//6
...
DtmfDetector::DtmfDetector(INT32 frameSize_): frameSize(frameSize_)
{
    ...
    prevDialButton = ' ';
    permissionFlag = 0;
	prevDialButton1 = prevDialButton2 = ' ';
	button1Count = button2Count = 0;
	nullCount = 0;
#if DEBUG
	//dtmfdebug = new ofstream("output_data",ios::app);
	time_t timer;  
    struct tm *tblock;  
    timer=time(NULL);  
    tblock=localtime(&timer);  
    sprintf(buf,"\n########## time is: %s",asctime(tblock));  
	//sprintf(filename, "data\\log\\logdtmf-%d%d%d-%d%d%d", tblock->tm_year+1900,tblock->tm_mon+1,tblock->tm_mday,
	//	tblock->tm_hour,tblock->tm_min,tblock->tm_sec);
	GenerateFileName("logdtmf", filename);
	dtmfdebug = new ofstream(filename, ios::app);
	dtmfdebug->write(buf,strlen(buf));
	GenerateFileName("result_dtmf", filename);
	dtmfcode = new ofstream(filename, ios::app);
//	dtmfcode->write(buf, strlen(buf));
#endif
}
//---------------------------------------------------------------------
...
extern void updateDTMFCode(char* str);
void DtmfDetector::dtmfDetecting(INT16 input_array[],int length)
{
...
//			NNABBBBCNN what !!!!!
			...
			if (temp_dial_button == ' ')
			{
				...
				if (nullCount >= 2)
				{
					if (button1Count >= 2 || button2Count >= 2)
					{
						...
						{
							dialButtons[indexForDialButtons++] = prevDialButton1;
							// NUL-terminate the string.
							dialButtons[indexForDialButtons] = 0;
						}
						else
						{
							dialButtons[indexForDialButtons++] = prevDialButton2;
							// NUL-terminate the string.
							dialButtons[indexForDialButtons] = 0;
						}
					}
			...
			else
			{
				if (nullCount >= 2)
				{
					nullCount = 0;
					button1Count=1;
					prevDialButton1 = temp_dial_button;
				}
				else
				{
					if (prevDialButton1 == ' ' && prevDialButton2 == ' ')
					{
						prevDialButton1 = temp_dial_button;
						button1Count++;
					}
					...
					{
						if (temp_dial_button == prevDialButton1)
						{
							button1Count++;
						}
						else if(temp_dial_button == prevDialButton2)
						{
							button2Count++;
							prevDialButton2 = temp_dial_button;
						}
						else
						{
							if (button1Count > button2Count)
...
//    printf("\n");
    ...
    for (ii = 0; ii < COEFF_NUMBER; ++ii)
	{
		sprintf(buf,"%s %d",buf,T[ii]);
        //dtmfdebug->write(T[ii]);
	}
	sprintf(buf,"%s \n",buf);
    dtmfdebug->write(buf,strlen(buf));
...
void dtmf_debug(char* str)
{
#if DEBUG
	dtmfdebug->write(str,strlen(str));
#endif
}
