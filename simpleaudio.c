...
ssize_t
simpleaudio_read( simpleaudio *sa, void *buf, unsigned int nframes )
{
    return sa->backend->simpleaudio1_read(sa, buf, nframes);
}

ssize_t
simpleaudio_write( simpleaudio *sa, void *buf, unsigned int nframes )
{
    return sa->backend->simpleaudio1_write(sa, buf, nframes);
}

void
simpleaudio_close( simpleaudio *sa )
{
    sa->backend->simpleaudio1_close(sa);
    free(sa);
}
